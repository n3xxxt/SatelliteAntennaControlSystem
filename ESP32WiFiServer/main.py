...
angle = ADC(Pin(34))
angle.atten(ADC.ATTN_0DB)

horizont = ADC(Pin(35))
horizont.atten(ADC.ATTN_0DB)
...
def check_system_response():    
    ACTUAL_ANGLE_VALUE = (90/87) * angle.read()/4095 * 90
    ACTUAL_HORIZONT_VALUE = (180/176) * (horizont.read()/4095) * 180
...
def rotate(type, pin, angle_pin, start_value, finish_value):
    

    if type == 0:
        info = (90/87) * (angle_pin.read()/4095) * 90

        
        if (start_value - finish_value) > 0.1:
            
            led.value(True)
            while info > finish_value:
        ...
        info = (180/176) * (angle_pin.read()/4095) * 180
...
def angle_rotate(value: float):
    angle_info = (90/87) * (angle.read()/4095) * 90
    print(angle_info)
    ...
    if angle_info < value:
        rotate(0, up, angle, angle_info, value)
    else:
        rotate(0, down, angle, angle_info, value)
     
    return True
    
def horizont_rotate(value:float):
    horizont_info = (180/176) * (horizont.read()/4095) * 180
    print(horizont_info)
    ...
    if horizont_info < value:
        rotate(1, up, horizont, horizont_info, value)
    else:
        rotate(1, down, horizont, horizont_info, value)
